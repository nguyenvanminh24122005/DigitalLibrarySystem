...
const API_BASE = import.meta.env.VITE_CATALOG_API || 'http://localhost:5002/api';
...
export async function getProfile(userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/profile`);
  return data;
}

export async function updateProfile(userId = USER_ID, profile) {
  return await request(`/users/${encodeURIComponent(userId)}/profile`, { method: 'PUT', body: JSON.stringify(profile) });
}

export async function getFavoriteBooks(userId = USER_ID) {
  try {
    const data = await request(`/users/${encodeURIComponent(userId)}/favorites`);
...
export async function addFavoriteBook(bookId, userId = USER_ID) {
  return await request(`/users/${encodeURIComponent(userId)}/favorites/${bookId}`, { method: 'POST' });
}

export async function removeFavoriteBook(bookId, userId = USER_ID) {
  return await request(`/users/${encodeURIComponent(userId)}/favorites/${bookId}`, { method: 'DELETE' });
}

export async function getCurrentBorrowings(userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/borrowings/current`);
  const list = Array.isArray(data) ? data : data?.items || [];
  return list.map(normalizeLoan);
}

export async function getBorrowingHistory(userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/borrowings/history`);
  const list = Array.isArray(data) ? data : data?.items || [];
  return list.map(normalizeLoan);
}

export async function borrowBookReal(bookId, copyId, userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/borrowings`, { method: 'POST', body: JSON.stringify({ bookId: Number(bookId), copyId: Number(copyId), days: 14 }) });
  return normalizeLoan(data);
}

export async function returnLoanReal(loanId, userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/borrowings/${loanId}/return`, { method: 'PUT' });
  return normalizeLoan(data);
}

export async function renewLoanReal(loanId, userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/borrowings/${loanId}/renew`, { method: 'PUT' });
  return normalizeLoan(data);
}

export async function approveBorrowingRequest(loanId, userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/borrowings/${loanId}/approve`, { method: 'PUT' });
  return normalizeLoan(data);
}

export async function rejectBorrowingRequest(loanId, reason = '', userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/borrowings/${loanId}/reject`, { method: 'PUT', body: JSON.stringify({ reason }) });
  return normalizeLoan(data);
}

export async function getNotifications(userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/notifications`);
  const list = Array.isArray(data) ? data : data?.items || [];
  return list.map(x => ({
    id: x.id,
    type: x.type || 'system',
...
export async function markAllNotificationsRead(userId = USER_ID) {
  return await request(`/users/${encodeURIComponent(userId)}/notifications/read-all`, { method: 'PUT' });
}

export async function getReadingProgress(userId = USER_ID) {
  const data = await request(`/users/${encodeURIComponent(userId)}/reading-progress`);
  const result = {};
...
export async function updateReadingProgress(bookId, progress, userId = USER_ID) {
  return await request(`/users/${encodeURIComponent(userId)}/reading-progress/${bookId}`, { method: 'PUT', body: JSON.stringify({ progress }) });
}
