...
import { catalogApi, getErrorMessage, unwrap } from '../services/api'
...
const availableCopies = computed(() => Number(availability.value.availableCopies ?? copies.value.filter((x) => x.borrowStatusValue === 1).length))
const borrowedCopies = computed(() => Number(availability.value.borrowedCopies ?? copies.value.filter((x) => x.borrowStatusValue === 2).length))
...
async function loadData() {
  try {
    ...
    copies.value = copiesRes.status === 'fulfilled' ? (unwrap(copiesRes.value) || []).map(mapCopy) : []
    availability.value = availabilityRes.status === 'fulfilled' ? unwrap(availabilityRes.value) || {} : {}
    history.value = historyRes.status === 'fulfilled' ? unwrap(historyRes.value) || [] : []
  } catch (e) {
