    ...
    ILogger<ReportEventSyncBackgroundService> logger) : BackgroundService
{
    ...
    private async Task<(int CatalogAdded, int CirculationAdded)> SyncOnceAsync(CancellationToken cancellationToken)
    {
        ...
        var catalogEvents = await GetArrayFromService("CatalogService", "/api/catalog-events?take=500", cancellationToken);
        var circulationEvents = await GetArrayFromService("CirculationService", "/api/circulation/history", cancellationToken);
        var catalogAdded = 0;
